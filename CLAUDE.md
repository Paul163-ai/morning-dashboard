...
**Guest (logged-out) view:** `require_auth()` lets unauthenticated visitors load `index.php` instead of redirecting to `/login.php`, plus logged-out GET requests to the read-only APIs `spurgeon.php`, `spurgeon_modern.php`, `bible.php` and `systematics.php`. `index.php` detects this via `is_authenticated()`, sets `window.IS_GUEST = true`, and forces a fixed config: the `daily`, `spurgeon`, `systematics`, `bible` and `resources` tabs, light theme, no prefs loaded/saved, no settings modal, "Log in" link instead of user/logout, plus a "Log in for more features" footer. `app.js` checks `IS_GUEST` to skip the Spurgeon notes textarea and Community Comments panel and their per-user API calls (`spurgeon_notes.php`, `spurgeon_comments.php`) that would 401, and keeps the guest's Bible translation in `localStorage` instead of prefs (the Spurgeon version choice isn't saved for guests). Guests get ESV on the site-wide key (deliberate; they share its rate limit) but never API.Bible, which needs a per-user key.
